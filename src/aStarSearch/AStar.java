...
public class AStar {
    ...
    static void checkAndUpdateCost(Cell current, Cell next, int cost){
        if(next == null || closed[next.i][next.j]) return;
        int t_final_cost = next.heuristicCost+cost;
        
        boolean inOpen = open.contains(next);
        if(!inOpen || t_final_cost < next.finalCost){
            next.finalCost = t_final_cost;
            next.parent = current;
    ...
    public static void test(int tCase, int x, int y, int si, int sj, int ei, int ej, int[][] blocked){
           ...
           for(int i=0; i < x; ++i){
              for(int j=0; j < y; ++j){
                  ...
                  grid[i][j].heuristicCost = Math.abs(i-endI -1)+Math.abs(j-endJ + 1);
